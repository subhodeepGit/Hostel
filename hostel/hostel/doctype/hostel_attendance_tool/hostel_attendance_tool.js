...
erpnext.employee_attendance_tool = {
	load_employees: function(frm) {
		if(frm.doc.date) {
			frappe.call({
				method: "hostel.hostel.doctype.hostel_attendance_tool.hostel_attendance_tool.get_employees",
				args: {
					//input data
					date: frm.doc.date,
	...
	make(frm, employee) {
		...
		mark_employee_toolbar.find(".btn-mark-present")
			...
			.on("click", function() {
				...
				frappe.call({
					method: "hostel.hostel.doctype.hostel_attendance_tool.hostel_attendance_tool.mark_employee_attendance",
					args:{
