...
class HostelSuspensionRoomChange(Document):
	...
	def before_insert(doc):
		...
		info_Room_Change=frappe.db.sql("""SELECT * from `tabHostel Suspension Room Change` where `allotment_number`="%s" and 
											`indisciplinary_actions_id`="%s" """%(allotment_number,indisciplinary_actions_id))
		if len(info_Room_Change)!=0:	
			if doc.allotment_type=="Hostel suspension":	
				info_al_HSRCH=frappe.db.sql("""SELECT * from `tabHostel Suspension Room Change` where 
									`allotment_number`="%s" and `indisciplinary_actions_id`="%s" """%(allotment_number,indisciplinary_actions_id))	
